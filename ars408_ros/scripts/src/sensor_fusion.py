...
class SensorFusion():
    ...
    def callback(self, radar_points, calib_rgb, name):
        rospy.loginfo("=="*5 + f"{name} CALLBACK" + "=="*5)
        
        self.fusion_data["radar"][name] = radar_points
        rgb_img = self.bridge.imgmsg_to_cv2(calib_rgb)
        bbox_array = self.yolo_model.inference(rgb_images=rgb_img, rgb_names=name)
        
        objects = Objects()
        if len(bbox_array) == 0 or len(self.fusion_data["radar"][name].rps) == 0:
            return
