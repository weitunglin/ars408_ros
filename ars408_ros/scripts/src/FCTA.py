...
from math import sqrt, pow, acos, sin, ceil
import rospy
...
class FCTAState(Enum):
    WARNING = 0
    DANGER = 1


class FCTAResult(object):
    def __init__(self, state: FCTAState, object: Object, ttr: float, ttc: float):
        self.state = state
        self.object = object
        self.ttr = ttr
        self.ttc = ttc
...
class FCTA():
    ...
    def __init__(self):
        ...
        self.pub_img = rospy.Publisher("/FCTA/img", Image, queue_size=1)
    ...
    def callback(self, radar_array: RadarPoints, speed: VehicleSpeedRpt, img: Image):
        ...
        for i in object_array.objects:
            ...
            v_ego = ceil(speed.vehicle_speed)
            ...
            """test data
            target:
                dist    = (  2, -10) # front left of ego vehicle
                vel     = (  0,   5) # moving right
            
            ego:
                vel     = (  2,   0) # moving forward
            
            ttr_target  = abs(distY / vrelY) = abs(-10/5) = 2
            colli_x     = distX + (vrelX + v_ego) * ttr_target = 2 + (2) * 2 = 6
            ttr_ego     = colli_x / v_ego = 3
            """
        ...
        for it in self.MonitoringList:
            if it['ContinuousAppearingCounter'] == self.BrakeTriggeringThreshold:
                flg = 1
                node = it
                break # avoid being overwritten by alert state
            if it['ContinuousAppearingCounter'] >= self.DangerAlertingThreshold:
                flg = 2
                node = it

        if flg == 1:
            rospy.logwarn("FCTA ENFORCES BRAKE")
            status = "FCTA ENFORCES BRAKE"
        ...
        # marker_array.markers.append(Marker(header=Header(stamp=rospy.Time.now(), frame_id="base_link"), ns="FCTA_text", type=Marker.TEXT_VIEW_FACING, action=Marker.DELETEALL))
        ...
        #     text = "FCTA " + ("Warning " if i.state == FCTAState.WARNING else "Danger ")
